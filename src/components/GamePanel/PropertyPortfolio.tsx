            ...
            <div key={group} className="animate-portfolio-slide">
              ...
              <div className="grid gap-2">
                {props.map((sq) => {
                  ...
                  const mortgageValue = sq.price ? Math.floor(sq.price / 2) : null;
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <div className="ml-2">
                            {sq.mortgaged ? (
                              ...
                              <button
                                disabled={!!(sq.colorGroup && props.some((p) => p.houses > 0))}
                                onClick={() => mortgageProperty(sq.id)}
                                className={`rounded-lg px-2 py-1 text-xs font-semibold ${sq.colorGroup && props.some((p) => p.houses > 0) ? 'bg-white/6 text-white/50' : 'bg-indigo-600 text-white'}`}
                              >Mortgage</button>
                            )}
                          </div>
                        </div>
